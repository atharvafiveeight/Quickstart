...
/**
 * RED_DEC_SHORT_AUTO - Short Autonomous Program for December Robot (Red Side)
 * 
 ...
 * @version 2.0
 */
@Autonomous(name = "RED_DEC_SHORT_AUTO", group = "Autonomous")
@Configurable
public class RED_DEC_SHORT_AUTO extends OpMode {
    ...
    private static final double AUTONOMOUS_SPEED_MULTIPLIER = 0.7;  // 70% of max speed (conservative for accuracy)
    ...
    public void init() {
        ...
        // Set maxPower to 70% for conservative, accurate movement
        ...
        telemetry.addData("Status", "RED_DEC_SHORT_AUTO Initialized");
        telemetry.update();
    }
        ...
        switch (pathState) {
            ...
            case 1:
                ...
                if (!follower.isBusy()) {
                    ...
                    telemetry.addData("DEBUG", "Arrived at first shot pose, starting to shoot preloaded balls");
                }
                break;
            ...
            case 7:
                // Move to shotPose2 while pre-warming shooters (75% velocity) and running intake at 0.7
                if (shotPose2 != null) {
                    ...
                    follower.followPath(shotPose2);
                    setPathState(8);
                    telemetry.addData("DEBUG", "Moving to shotPose2 with pre-warm and intake at 0.7");
                }
                break;
            ...
            case 8:
                ...
                if (!follower.isBusy()) {
                    ...
                    telemetry.addData("DEBUG", "Arrived at shotPose2, finishing spin-up and shooting");
                }
                break;
            ...
            case 14:
                // Move to shotPose3 while pre-warming shooters (75% velocity) and running intake at 0.7
                if (shotPose3 != null) {
                    // Start pre-warming shooters
                    startShooterPreWarm();
                    // Start intake at 0.7 power
                    ...
                    follower.followPath(shotPose3);
                    setPathState(15);
                    telemetry.addData("DEBUG", "Moving to shotPose3 with pre-warm and intake at 0.7");
                }
                break;
            ...
            case 15:
                ...
                if (!follower.isBusy()) {
                    ...
                    setPathState(16);
                    setShootingState(1); // Start shooting
                    telemetry.addData("DEBUG", "Arrived at shotPose3, finishing spin-up and shooting");
                }
                break;
            ...
            case 21:
                // Move to shotPose4 while pre-warming shooters (75% velocity) and running intake at 0.7
                if (shotPose4 != null) {
                    // Start pre-warming shooters
                    startShooterPreWarm();
                    // Start intake at 0.7 power
                    ...
                    follower.followPath(shotPose4);
                    setPathState(22);
                    telemetry.addData("DEBUG", "Moving to shotPose4 with pre-warm and intake at 0.7");
                }
                break;
            ...
            case 22:
                ...
                if (!follower.isBusy()) {
                    ...
                    setPathState(23);
                    setShootingState(1); // Start shooting
                    telemetry.addData("DEBUG", "Arrived at shotPose4, finishing spin-up and shooting");
                }
                break;
                
            case 23:
                // Wait for fourth shooting round to complete
                if (shootingComplete) {
                    setPathState(24);
                    shootingComplete = false; // Reset for next round
                    telemetry.addData("DEBUG", "Fourth shooting round complete, moving to startPoseTeleop");
                }
                break;
                
            case 24:
                // Move to startPoseTeleop - final position for teleop
                if (startPoseTeleop != null) {
                    follower.followPath(startPoseTeleop);
                    setPathState(25);
                    telemetry.addData("DEBUG", "Moving to startPoseTeleop");
                }
                break;
                
            case 25:
                // Wait for arrival at startPoseTeleop, then finish
                if (!follower.isBusy()) {
                    stopIntake();
                    setPathState(-1);
                    telemetry.addData("DEBUG", "Arrived at startPoseTeleop, autonomous complete!");
                }
                break;
        }
    }
    ...
    public void autonomousShootingUpdate() {
        switch (shooterState) {
            ...
            case SPIN_UP:
                ...
                // Run intake at 0.85 power while spinning up (helps prepare balls)
                if (intakeMotor != null) {
            ...
            case READY:
                ...
                // Continue intake at 0.85 power while feeding
                if (intakeMotor != null) {
                    intakeMotor.setPower(INTAKE_SHOOTING_POWER);
                }
    ...
    private void updateTelemetry() {
        ...
        telemetry.addLine("=== RED DEC SHORT AUTO ===");
