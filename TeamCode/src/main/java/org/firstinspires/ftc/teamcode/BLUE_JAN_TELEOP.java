...
public class BLUE_JAN_TELEOP extends LinearOpMode {
    ...
    // Feed Motor Constants - now dynamic based on R1 (0.8) or R2 (0.3)
    // Feed motor power is set dynamically: R1 (right bumper) = 0.8, R2 (right trigger) = 0.3
    ...
    private static final double[][] VELOCITY_LOOKUP_TABLE = {
        ...
        // Long distances (> 80") - start at 100" with 1400 RPM, then interpolate from 102.5"
        // All velocities increased by 50 RPM for better long distance performance
        {100.0, 1450.0}, // Starting point for long distances: 1450 RPM at 100" (was 1400)
        {102.5, 1465.0}, // Continue with existing interpolation from 102.5" (was 1415)
        {104.2, 1475.0}, // Calculated: linear interpolation between 102.5" (1465) and 126" (1600) (was 1425)
        {106.5, 1488.0}, // Calculated: linear interpolation between 102.5" (1465) and 126" (1600) (was 1438)
        {126.0, 1600.0}  // End point: 1600 RPM at 126" (was 1550)
    };
    ...
    private static final double SHOOTER_PIDF_P = 130.0;  // Proportional gain (increased from 100 for faster velocity recovery)
    ...
    private static final double SHOOTER_PIDF_F = 40.0;   // Feedforward gain (increased from 35 for better base power under load)
    ...
    private double currentFeedMotorPower = 0.3;  // Current feed motor power (set by R1=0.8 or R2=0.3)
    ...
    private void handleShooterControls() {
        ...
        if (r1CurrentlyPressed) {
            ...
            currentFeedMotorPower = 0.3;  // R2 uses 0.3 power
        }
    ...
    private void updateTelemetry(double flPower, double frPower, double blPower, double brPower, double botHeading) {
        ...
        // 2. IMU Status
        if (drive != null) {
            telemetry.addData("IMU Status", "OK (PedroPathing)");
        } else {
            telemetry.addData("IMU Status", "NOT INITIALIZED");
        }
        
        // 3. Distance measured and 4. Target velocity
        if (limelight != null) {
            LLResult result = limelight.getLatestResult();
            if (result != null && result.isValid()) {
                double ta = result.getTa();
                ...
                if (rgb != null) {
                    try {
                        ...
                        // Silently fail - don't show error in simplified telemetry
                    }
                }
                ...
                // Show distance
                telemetry.addData("Distance", String.format("%.1f\"", actualDistance));
                
                // Calculate and show target velocity
                double calculatedVelocity = lookupVelocity(actualDistance);
                calculatedVelocity = Math.max(MIN_VELOCITY, Math.min(MAX_VELOCITY, calculatedVelocity));
                
                telemetry.addData("Target Velocity", String.format("%.0f RPM", calculatedVelocity));
            } else {
                ...
                telemetry.addData("Distance", "N/A (no target)");
                telemetry.addData("Target Velocity", String.format("%.0f RPM (default)", SHOOTER_TARGET_VELOCITY));
            }
        } else {
            ...
            telemetry.addData("Distance", "N/A (Limelight disconnected)");
            telemetry.addData("Target Velocity", String.format("%.0f RPM (default)", SHOOTER_TARGET_VELOCITY));
        }
