...
@Autonomous(name = "RED-SHORT-NOVAUTO", group = "Autonomous")
public class RED_SHORT_NOVAUTO extends OpMode {
    ...
    // Robot positions on the field (in inches)
    private final Pose startPose = new Pose(86.795, 134.575, Math.toRadians(0)); // Where robot starts
    private final Pose scorePose = new Pose(82.192, 97.534, Math.toRadians(40)); // Where robot shoots
    private final Pose startTeleopPose = new Pose(81.096, 38.795, Math.toRadians(0)); // Where robot ends
    ...
    private void initializeLauncherHardware() {
        try {
            ...
            telemetry.addData("Status", "Shooter hardware ready");
        } catch (Exception e) {
    ...
    private void buildAutonomousPaths() {
        try {
            // Path 1: Move from start position to shooting position
            startToScore = follower.pathBuilder()
                    .addPath(new BezierLine(startPose, scorePose))
                    .setLinearHeadingInterpolation(startPose.getHeading(), scorePose.getHeading())
                    .build();
            telemetry.addData("Status", "Path 1 created: Start to Shooting");
                    
            // Path 2: Move from shooting position to final position
            scoreToTeleop = follower.pathBuilder()
                    ...
                    .build();
            telemetry.addData("Status", "Path 2 created: Shooting to Final");
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            case 0:
                // Move from start to shooting position
    ...
    /**
     * Control the shooter motor and ball feeding
     */
            ...
            case LAUNCHING:
                ...
                try {
                    ...
                    if (feederTimer.seconds() > FEED_TIME_SECONDS) {
                        ...
                        telemetry.addData("Ball " + ballsScored + " Shot", "Velocity: " + String.format("%.1f", launchingVelocity));
    ...
    private void updateTelemetry() {
        ...
        // Header
        telemetry.addLine("=== AUTONOMOUS RED ===");
        telemetry.addLine();
