...
@Autonomous(name = "BLUE-SHORT-NOVAUTO", group = "Autonomous")
public class BLUE_SHORT_NOVAUTO extends OpMode {
    ...
    // Blue-side poses from NovAutoBlue
    private final Pose startPose = new Pose(57.205, 134.575, Math.toRadians(180));
    private final Pose scorePose = new Pose(61.808, 97.534, Math.toRadians(140));
    ...
    private void stopLauncher() {
        try { shooterMotor.setVelocity(0); leftServo.setPower(STOP_SPEED); rightServo.setPower(STOP_SPEED); launchState = LaunchState.IDLE; } catch (Exception ignored) {}
    }

    private void updateTelemetry() {
        telemetry.clear();
        telemetry.addLine("=== AUTONOMOUS BLUE ===");
